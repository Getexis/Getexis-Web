...
class Shiba_Media_Library_Add {
	...
	function Shiba_Media_Library_Add() {
		...
		add_meta_box(	'post-content-div', __('Descrição da Galeria', THEMEDOMAIN), array(&$this, 'gallery_description_metabox'), 'gallery', 'normal', 'high');
...
//		if (!isset($_GET['post_type']) || ($_GET['post_type'] != 'gallery')) return;
		...
		<h3 style="float:left;padding-right:20px;">Imagens da Galeria</h3>
		...
		<form id="gallery-add-image" action="media-upload.php" method="get"> 
			...
			<input type="button" value="<?php esc_attr_e('Adcionar Novas Imagens'); ?>" id="addimage-to-gallery" name="addimage-to-gallery" class="button-secondary action"/>
		</form>
		</div>
		...
		<form id="gallery-list" action="upload.php" method="<?php if (class_exists('WP_Media_List_Table')) echo 'post'; else echo 'get';?>">
			...
			<option value="-1" selected="selected"><?php _e('Mais Ações', THEMEDOMAIN); ?></option>
			<option value="remove"><?php _e('Remover da Galeria', THEMEDOMAIN); ?></option>
			<?php if (function_exists('wp_trash_post') && !$is_trash) { ?>
			<option value="trash"><?php _e('Deletar para Lixeira', THEMEDOMAIN); ?></option>
			<?php } ?>            
			</select>
...
//		$this->display_tablenav( 'bottom' );
	...
	function gallery_description_metabox($post) {
        ...
        <textarea rows="7" cols="32" name="content" tabindex="6" id="content"><?php echo $post->post_content; ?></textarea>
		<?php
	}
