...
function post_type_projetos() {
	$labels = array(
    	'name' => _x('Projetos', 'post type general name', THEMEDOMAIN),
    	'singular_name' => _x('Projeto', 'post type singular name', THEMEDOMAIN),
    	'add_new' => _x('Adicionar Novo Projeto', 'book', THEMEDOMAIN),
    	'add_new_item' => __('Adicionar Novo Projeto', THEMEDOMAIN),
    	'edit_item' => __('Editar Projeto', THEMEDOMAIN),
    	'new_item' => __('Adicionar Projeto', THEMEDOMAIN),
    	'view_item' => __('Visualizar Projeto', THEMEDOMAIN),
    	'search_items' => __('Pesquisar Projeto', THEMEDOMAIN),
    	'not_found' =>  __('Projeto Não Encontrado', THEMEDOMAIN),
    	'not_found_in_trash' => __('Projeto Não Encontrado no Lixo', THEMEDOMAIN), 
    	'parent_item_colon' => ''
	);		
	$args = array(
    	'labels' => $labels,
	...
	register_post_type( 'projetos', $args );
  	...
  	$labels = array(			  
  	  'name' => _x( 'Categoria de Projeto', 'taxonomy general name', THEMEDOMAIN ),
  	  'singular_name' => _x( 'Categoria de Projeto', 'taxonomy singular name', THEMEDOMAIN ),
  	  'search_items' =>  __( 'Pesquisar Categoria de Projeto', THEMEDOMAIN ),
  	  'all_items' => __( 'Todas Categorias de Projeto', THEMEDOMAIN ),
  	  'parent_item' => __( 'Parent Portfolio Set', THEMEDOMAIN ),
  	  'parent_item_colon' => __( 'Parent Portfolio Set:', THEMEDOMAIN ),
  	  'edit_item' => __( 'Editar Categoria de Projeto', THEMEDOMAIN ), 
  	  'update_item' => __( 'Atualizar Categoria de Projeto', THEMEDOMAIN ),
  	  'add_new_item' => __( 'Adicionar Categoria de Projeto', THEMEDOMAIN ),
  	  'new_item_name' => __( 'Adcionar Nova Categoria de Projeto', THEMEDOMAIN ),
  	); 							  
  	...
  	register_taxonomy(
		...
		array(
			...
			'hierarchical' => true,
			'labels'=> $labels,
			'query_var' => 'projetossets',
			'show_ui' => true,
			'rewrite' => array( 'slug' => 'projetossets', 'with_front' => false ),
		)
	);		  
} 
...
add_filter( 'manage_posts_columns', 'rt_add_gravatar_col_new');
function rt_add_gravatar_col_new($cols) {
	$cols['thumbnail'] = __('Thumbnail', THEMEDOMAIN);
	return $cols;
}

add_action( 'manage_posts_custom_column', 'rt_get_author_gravatar_new');
function rt_get_author_gravatar_new($column_name ) {
...
$postmetas = 
	array (
		...
		'projetos' => array(
			array("section" => "Content Type", "id" => "projeto_type", "type" => "select", "title" => "Tipos:", "description" => "Modo de Visualização:", 
				"items" => array(
...
function create_meta_box_new() {
	...
	if ( function_exists('add_meta_box') && isset($postmetas) && count($postmetas) > 0 ) {  
		foreach($postmetas as $key => $postmeta)
		{
			...
			{
				add_meta_box( 'metabox', ucfirst($key).' Options', 'new_meta_box_new', $key, 'normal', 'high' );  
			}
		}
	}
}  */
/*
function new_meta_box_new() {
...
function save_postdata_new( $post_id ) {
	...
	foreach ( $postmetas as $postmeta ) {
		foreach ( $postmeta as $each_meta ) {
			...
			if (isset($_POST[$each_meta['id']]) && $_POST[$each_meta['id']]) {
				update_custom_meta_new($post_id, $_POST[$each_meta['id']], $each_meta['id']);
			}
...
/*
function update_custom_meta_new($postID, $newvalue, $field_name) {
...
//add_action('/admin_menu', 'create_meta_box_new'); 
add_action('save_post', 'save_postdata_new');  
